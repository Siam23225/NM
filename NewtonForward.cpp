...
void finiteDifferenceTable(vector<vector<double>>& diff_table, const vector<double>& y, int n) {
    for (int i = 0; i < n; i++)
        diff_table[i][0] = y[i];
    for (int j = 1; j < n; j++) {
        for (int i = 0; i < n - j; i++) {
...
double newtonForwardInterpolation(const vector<double>& x, const vector<vector<double>>& diff_table, int n, double x_interp) {
    double h = x[1] - x[0];
...
int main() {
    ...
    finiteDifferenceTable(diff_table, y, n);
    ...
    double result = newtonForwardInterpolation(x, diff_table, n, x_interp);
